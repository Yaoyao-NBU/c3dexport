...
将 **8 通道 Kistler（Type 3）** C3D 转换为 OpenSim 文件，使用**阈值法**进行站立相检测。
...
将 **6 通道 AMTI（Type 1）**测力台 C3D 转换为 OpenSim 文件。

Type 1 测力台直接输出力和压力中心坐标（Px, Py 为 COP 坐标，Mz 为自由垂直力矩），无需力矩到 COP 的转换。

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `c3d_path` | `str` | 必填 | 含 6 通道 Type 1 力数据的 `.c3d` 文件路径 |
| `output_dir` | `str` | 必填 | 输出文件目录 |
| `marker_cutoff` | `float` | `6.0` | 标记点低通滤波截止频率（Hz） |
| `force_cutoff` | `float` | `50.0` | 力数据低通滤波截止频率（Hz） |
| `stance_threshold` | `float` | `30.0` | 站立相检测的垂直力阈值（N） |
...
**返回值：** `dict`，与 `convert_c3d_type3` 相同的键。

---

#### `convert_c3d_type2(c3d_path, output_dir, ...)`

将 **6 通道 AMTI（Type 2）**测力台 C3D 转换为 OpenSim 文件。
